...
The Doxygen class documentation is built and published by CI (the "Deploy
docs to GitHub Pages" workflow), so `graphviz` and `doxygen` are no longer
release-time prerequisites.
...
2. Bump the project version number with [bumpversion](https://pypi.org/project/bump2version/).
   ...
   To set a specific version, use `bumpversion --new-version x.y.z <part>` where `<part>` is
   any of `major`, `minor`, `patch`, or `release`.

   Bumpversion always creates a new commit with the new version number.

3. Update the autogenerated web UI source files:

       scripts/update_autogen.sh

   A new commit is generated if the web UI bundle changed. The Doxygen class
   documentation is no longer committed here -- it is regenerated and deployed
   to GitHub Pages automatically when the release is merged to `main`.
